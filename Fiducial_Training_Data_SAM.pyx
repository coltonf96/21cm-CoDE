...
import os, time, shutil, random, sys
cimport numpy as np
cimport cython
...
# ------ Above are the codes for determining PopIII M_star and PopII star formation, below is the full SAM framework for generating neural network training data --------------------------------

def LW_combined(int JLW_i, int vbc_i, int delta, str special='', int N_star=4, double star_m0=35.69, t_delay=1e7):
    cdef int checking, num_cells, im, iN, z_start, iz, N_halos, ii, ID, ID_prog1, ID_prog2, poisson_im, check
    ...
    cdef double V_com, fIII, z_pres, z_prev, time_z_arrays, H_0, h, Omega_b, Omega_m, Omega_L, E_SN, epsilon_SN, R_C_cgs, fit_life, fit_ion, fit_eta, start, J, M_H2, M_crit_0, Omega_z_m, del_c, t_pres, delta_t, tau_ff, m_gas_z, M_crit, M_halo, M_DM, M_baryon, M_stellar, M_ejected, M_gas_max, star_m, z_calc, Star_prog1, Star_prog2, popIIIprog1, popIIIprog2, t_popIII_on, eta_SN, popII_new, popIII_sfr_sum, popII_sfr_sum, popIII_sfr, popII_sfr, end
    cdef np.ndarray[np.int_t,ndim=1] poisson, all_ID, polluted, ID_present, last_ID, all_desc, ID_prog, prog_ff
    cdef np.ndarray[np.float64_t,ndim=1] z, z_ff, t, tau, mass_bins, sigmas, D, all_z, J_z, M_crit_z, M_a, x, cumulative, mass_of_stars, coeffs_life, coeffs_ion, coeffs_eta, SFRD_II, SFRD_III, MMH, m_gas, m_lost, popIII, popIII_on, popII, popIII_m, popII_m, t_quieses, fIIs, all_m, output, popIII_sfr_bar, popII_sfr_bar, M_star
    ...
    V_com, fIII = 27., 0.0001                   #Comoving volume & PopIII SFE
    H_0, Omega_b, Omega_m, E_SN, epsilon_SN = 67., 0.049, 0.32, 1e51, 1.6e-3  #Various constants
    ...
    eta_SN, T_vir_steady = 0., 1.76e4           #SNe gas ejection efficiency & T_vir for steady PopII SF
    ...
    if special == '_Mcrit':
        print('Multiplying M_crit by 2.25x')
        M_crit_z *= 2.25
        M_crit_z[M_crit_z > M_a] = M_a[M_crit_z > M_a]    #Replace values > M_atomic with M_a(z)
    print('# of stars & their mass: ', N_star, star_m0)

    coeffs_III = np.load('./Schaerer_PopIII_Fit_Coeffs.npy', allow_pickle=True)         #Fitting coefficients for the PopIII stellar lifetime...
    coeffs_life, coeffs_ion, coeffs_eta = coeffs_III[0], coeffs_III[1], coeffs_III[2]   #The # of ionizing photons [s^-1] & # of ion/LW photons per baryon
    fit_life, fit_ion, fit_eta = 0., 0., 0.                                             #Initialize fit value for each property
    ...
    print('(Log) Lifetime, Ionizing, Eta_LW = ', fit_life, fit_ion, fit_eta)
    fit_life, fit_ion, fit_eta = 10.**(fit_life), 10.**(fit_ion), 10.**(fit_eta)    #Raise fit to 10^fit
    ...
    del_c = 18.*(np.pi**2.) + 82.*(Omega_z_m-1.) - 39.*((Omega_z_m-1.)**2.)                     #EQ 22 "" -- Both used in escape velocity for halo (m_ej)
    print(J, M_crit, z_pres, t_pres)
        ...
        for iN in range(0, poisson_im):             #Loop calculating stellar mass (among other things) for every tree
            ...
            for ii in range(0, len(ID_present)):                #Loop through current halos for this tree
                ...
                if M_halo >= M_crit and polluted[ID] < 1e-8:    #Assigning PopIII stars to a halo as soon as it's > M_crit
                    star_m = star_m0 * float(N_star)            #Stellar mass to be introduced (four PopIII stars with M = 28.41 M_sun)
...
#    np.save(save_path + '/SF_' + str(vbc_i) + '_' + str(JLW_i) + special + '.npy', np.row_stack((M_burst, M_steady, M_star))) #Finally, save star formation histories
    np.save('/fs/scratch/PJS0312/coltonf96/Delta_' + str(delta) + '/SF_' + str(vbc_i) + '_' + str(JLW_i) + special + '.npy', np.row_stack((M_burst, M_steady, M_star)))
    end = time.time() - start
