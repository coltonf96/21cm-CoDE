...
def Get_Data(int popIII, int popII, int delta, str special=''):   # MAKE SURE THAT YOU RUN COMBINE_SF_OUTPUTS.PY FIRST ------------
  cdef int i, iz, delay_iz
  cdef str suffix
  cdef double t_delay, vbc, z_pres, int_M, int_J, t_delay_ago, z_delay_ago, N_delay, M_crit_ago, int_N_t
  cdef np.ndarray[np.float64_t,ndim=1] z, t, vbcs, J_z, M_crit_z, MMH, cM_star, new_row, SF_II
  ...
  t_delay = 1e7                         #Delay time between PopIII & PopII star formation
  ...
  if popIII == 1:                       #If we need to gather PopIII data...
    ...
    for i in range(0, len(all_inputs)):                                       #Loop through all LW/vbc combos
      ...
      cM_star, vbc = SF[3,:], round(vbcs[inputs[0]],2)              #Cumulative (smoothed) M_PopIII and vbc parameter in terms of sigma_vbc
      for iz in range(0, len(cM_star)-10):              #Now loop back through time to gather PopIII SF data
        if cM_star[iz] < 1e-15:               #Whenever we reach redshifts with no SF, skip them
          break
  ...
  if popII == 1:                  #Now if we need to gather PopII data...
    all_data = []                 #Initialize list of all data values
    ...
    for i in range(0, len(all_inputs)):                                       #Now loop through all LW/vbc combos
      ...
      SF_II, cM_star = SF[1,:], SF[3,:]
      for iz in range(len(z)-10, -1, -1):           #Now step FORWARD through time
        if SF_II[iz] < 1e-15:                       #...until we get to PopII SF
            continue
        z_pres = round(z[iz],2)                     #Current redshift
        ...
        new_row = np.array([z_pres, np.log10(SF_II[iz]), np.log10(int_N_t), np.log10(M_crit_ago)])
        all_data.append(new_row)    #Append to overall list
    all_data = np.array(all_data)   #Convert list into array
    print(all_data[0])
    print(all_data.shape)
    np.save('./Deltas/Delta_' + str(delta) + '/PopII_Box_Data' + special + '.npy', all_data, allow_pickle=True)
