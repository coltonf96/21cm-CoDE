...
class SimpleNet2(nn.Module):       #This is the Neural Net for PopII ---------------------------------------------------------------
    def __init__(self):
        super(SimpleNet2, self).__init__()  #An apparently necessary line -- super("ClassName", self).__init__()
        self.fc1 = nn.Linear(2, 50)         #Four input channel to 50 variables
...
def Get_J(double z_pres, np.ndarray[np.float64_t,ndim=1] z, np.ndarray[np.float64_t,ndim=1] t, np.ndarray[np.float64_t,ndim=1] f_LW, int N, np.ndarray[np.float64_t,ndim=1] LW_vals, np.ndarray[np.float64_t,ndim=4] SFR_III_0, np.ndarray[np.float64_t,ndim=4] SFR_II_0, int z_iz, str save_path, np.ndarray[np.float64_t,ndim=1] LW_dists, np.ndarray[np.float64_t,ndim=1] z_dists, int iz_stop):
    ...
    cdef double H_0_s, Omega_m, m_proton, eta_II, eta_III, E_LW, dnu_LW, c, N_cells, V_0, V_1, start_smooth, start_cube, start_interp, t_smooth_z, t_cube_z, t_interp_z
...
def Get_M_Crit(np.ndarray[np.float64_t,ndim=3] J, np.ndarray[np.float64_t,ndim=3] v_bc, double z, np.ndarray[np.float64_t,ndim=1] J0_M_crit, np.ndarray[np.int_t,ndim=3] all_vbc_i, double M_a):
    ...
    J_0_cube = J0_M_crit[all_vbc_i]             #3D array of M_Crit(J_LW=0) values by vbc
...
def Simulation(str special, str NN_special):
    cdef int N_side, N_EPOCHS_III, N_EPOCHS_II, iz, delay_iz, N_iz, i, ii, ij, ik, emulation, delta_i, iz_stop
    cdef str base_path, save_path, emulating, suffix
    cdef double start, t_delay, V_com, H_0, Omega_m, c, H_0_s, eta_II, eta_III, m_proton, h_planck, E_LW_l_eV, E_LW_u_eV, E_LW_l, E_LW_u, E_LW_eV, E_LW, nu_l, nu_u, dnu_LW, z_pres, dt, t_delay_ago, M_a, delta, int_M, int_J, log_M, log_J, log_J_LW, log_MMH, vbc, dII_dz, Mcrit_delay, log_N_t, start_e, start_II, start_III, start_crit, start_save
    cdef np.ndarray[np.int_t,ndim=2] cells
    cdef np.ndarray[np.int_t,ndim=3] mergers, all_delta_i, all_vbc_i
    cdef np.ndarray[np.float64_t,ndim=1] z, t, vbcs, vbc_z, LW_vals, MMH, inputs_III_0, M_crit_cell, J_LW_cell, inputs_II_0, N_fid_III, N_fid_II, t_recent, deltas, deltas0, t_z, t_LW, t_crit, t_interp, t_smooth, t_cube, t_save, t_III, t_II, N_III, N_II
    cdef np.ndarray[np.float64_t,ndim=2] J_0, MMHs
    cdef np.ndarray[np.float64_t,ndim=3] t_III_all, J, M_crit, all_vbc, all_delta
    cdef np.ndarray[np.float64_t,ndim=4] SFR_III, SFR_II, Pop_III, Pop_II, J_z, M_crit_z, Pop_III_Smooth, Pop_II_Smooth
    ...
    t_delay, V_com = 1e7, 3.0**3.0                          #Delay time for PopII SF & cell volume in Mpc^3
    ...
    Pop_II = np.zeros((N_side, N_side, N_side, len(z)))             #And the emulated SMD_II (PopII) values
    ...
    for iz in range(899, -1, -1):                       #Step through time and begin simulating
        z_pres, dt = round(z[iz], 2), t[iz]-t[iz+1]     #Current redshift step, and years since previous step
        t_delay_ago = t[iz] - 1e7                       #Hubble time one delay time ago
        ...
        M_crit = Get_M_Crit(J, all_vbc, z_pres, J_0[:,iz], all_vbc_i, M_a)  #Critical mass caluclation
        ...
        for delta_i in range(0, len(deltas)):               #Loop through each overdensity bin and emulate SF in all cells with that delta value
            ...
            model_III.eval()
            model_II = SimpleNet2().to(device=device)       #And set up trained NN emulation for PopII stars
            model_II.load_state_dict(torch.load(emulating + '/Emul_' + str(N_EPOCHS_II) + '_PopII' + NN_special + '.pt'))
            model_II.eval()
            MMH = MMHs[delta_i]                                     #Relevant MMH array for this overdensity bin merger history
            ...
            for i in range(0, len(cells[0])):                       #Loop through all of those cells in this bin
                ...
                # The following block of code is for calculating PopII star formation, above is PopIII ----------------------------------
                if t[iz] > (t_III_all[ii,ij,ik] + 1e7):                                         #After the delay time has passed, emulate PopII as well
                    ...
                    Pop_II[ii,ij,ik,iz] = max(10.**(model_II(inputs_II.float())), Pop_II[ii,ij,ik,iz+1])    #Emulate & record star formation (don't let it be less than previous z)
                    Pop_II_Smooth[ii,ij,ik,iz] = np.mean(Pop_II[ii,ij,ik,iz:iz+10])                         #Record smoothed value by averaging over last dz=0.5
                    SFR_II[ii,ij,ik,iz] = (np.floor(Pop_II_Smooth[ii,ij,ik,iz])-np.floor(Pop_II_Smooth[ii,ij,ik,iz+1])) / dt      #Calculate PopII SFR
        # Print out results every time step, and overwrite previous result arrays every dz = 0.5 -----------------------------------------
        print('Mean N_III & SF_II: ', np.mean(Pop_III[:,:,:,iz]), np.mean(Pop_II[:,:,:,iz]))
        ...
        if iz % 100 == 0:
            ...
            np.save(save_path + 'Mstar_II_all.npy', Pop_II)
